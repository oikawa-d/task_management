...
def _due_at_in_app_date(day_offset: int = 0) -> datetime:
	...
	local_date = datetime.now(timezone.utc).astimezone(app_timezone).date() + timedelta(days=day_offset)
...
async def test_sp_create_task_inserts_due_today_notification_for_assignee(db_session: AsyncSession) -> None:
	...
	assert notification == {
		...
		"body": "body",
		"due_at": due_at,
		"dedupe_key": f"created:{task_id}",
	}


async def test_sp_create_task_skips_due_today_notification_without_assignee_or_for_future(
...
async def test_sp_update_task_inserts_only_when_due_at_changes_to_today_and_deduplicates(
	db_session: AsyncSession,
) -> None:
	...
	notifications = (
		(
			await db_session.execute(
				text(
					"SELECT type, title, due_at, dedupe_key FROM notifications "
