...
| `RATE_LIMIT_EMAIL_VERIFY_MAX_REQUESTS` / `RATE_LIMIT_EMAIL_VERIFY_WINDOW_SECONDS` | `10` / `900` | メール認証のIP単位Rate Limit |
| `RATE_LIMIT_EMAIL_VERIFY_RESEND_MAX_REQUESTS` / `RATE_LIMIT_EMAIL_VERIFY_RESEND_WINDOW_SECONDS` | `5` / `900` | 認証メール再送のIP単位Rate Limit |
| `RATE_LIMIT_PASSWORD_FORGOT_MAX_REQUESTS` / `RATE_LIMIT_PASSWORD_FORGOT_WINDOW_SECONDS` | `5` / `900` | パスワード再設定要求のIP単位Rate Limit |
| `RATE_LIMIT_PASSWORD_RESET_MAX_REQUESTS` / `RATE_LIMIT_PASSWORD_RESET_WINDOW_SECONDS` | `10` / `900` | パスワード再設定実行のIP単位Rate Limit |
