...
| `RATE_LIMIT_EMAIL_VERIFY_WINDOW_SECONDS` / `RATE_LIMIT_EMAIL_VERIFY_RESEND_WINDOW_SECONDS` | `900` / `900` | メール認証・再送のRate Limit時間窓 |
| `RATE_LIMIT_PASSWORD_FORGOT_MAX_REQUESTS` / `RATE_LIMIT_PASSWORD_RESET_MAX_REQUESTS` | `5` / `10` | パスワード再設定要求・実行のIP単位Rate Limit |
| `RATE_LIMIT_PASSWORD_FORGOT_WINDOW_SECONDS` / `RATE_LIMIT_PASSWORD_RESET_WINDOW_SECONDS` | `900` / `900` | パスワード再設定要求・実行のRate Limit時間窓 |
