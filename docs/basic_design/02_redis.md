...
**`login_fail:{key_hash}` の許容回数**：`LOGIN_MAX_ATTEMPTS`（既定 `5`）を正式な閾値とする。`login_fail:{key_hash}` の値がこの値以上になったログイン試行は、`LOGIN_LOCK_WINDOW_SECONDS`（既定900秒）が経過するまで拒否する。`.env`/`core/config.py`で運用時に変更可能な環境変数として定義する（学習用途としての妥当性は運用開始時に再確認してよい）。
