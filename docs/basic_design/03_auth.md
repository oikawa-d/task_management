...
| 関数 | 引数 | 戻り値 | 処理 |
|------|------|--------|------|
| `register` | `payload: RegisterRequest`, `background: BackgroundTasks` | `User` | `api/app/service/auth_service.py`。重複チェック → ハッシュ化 → `users` INSERT（`email_verified_at=NULL`）→ 確認メール送信予約。**Strategy.login は呼ばない** |
