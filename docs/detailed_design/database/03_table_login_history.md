...
- issue #444（本書）で `failure_reason` の保存値を大文字から小文字snake_caseへ統一したが、既に大文字値でINSERT済みの既存 `login_history` レコードは本対応では移行しないため、大文字・小文字が混在した状態が残る。既存データのバックフィル方針は未決であり、issue #459 で決定する（要検討）。
