...
### 2.8 既存SPを`CREATE OR REPLACE`で変更するリビジョンのdowngrade実装方針

`0016`（issue #200）のように、既に別リビジョンが作成済みのSP（例：`0010`の`sp_create_task`/`sp_update_task`）を後続リビジョンで`CREATE OR REPLACE`により変更する場合、§2.3の「`op.execute()`でSQL資材ファイルを読み込む」方式をそのまま踏襲すると、後続リビジョンと元のリビジョンが同一の`db/functions/*.sql` / `db/procedures/*.sql`を参照してしまい、次の2点でリビジョンの再現性が崩れる。

- 空DBから`alembic upgrade head`した場合、SQL資材ファイルの内容（最新版）が元のリビジョンの時点で適用されてしまい、後続リビジョンが同一内容を再適用するだけになる（段階適用の意味が失われる）
- 後続リビジョンを`downgrade()`しても、SQL資材ファイルには旧定義が残っていないため、元のリビジョン相当の状態へ戻せない

そのため、既存SPを変更する後続リビジョンは、SQL資材ファイルを読み込まず、変更前・変更後両方のSQL本体をリビジョンファイル内にインライン文字列として自己完結させる。`upgrade()`は変更後（インライン新定義）を、`downgrade()`は変更前（インライン旧定義）を、それぞれ`op.execute()`で適用する。変更前の定義は、変更を加えたコミットの親コミット時点の`db/functions/*.sql` / `db/procedures/*.sql`（`git show <変更前コミット>:<パス>`）から取得する。

この方針を採る後続リビジョンでは、元のリビジョンが読み込む`db/functions/*.sql` / `db/procedures/*.sql`は変更前の定義のまま維持し、`head`到達後の実際の定義（変更後）と資材ファイルの内容が一致しなくなる。これは§2.3のDRY原則（SQL本体をリビジョンファイル内に重複させない）の例外であり、リビジョンの再現性を優先した意図的なトレードオフである。
