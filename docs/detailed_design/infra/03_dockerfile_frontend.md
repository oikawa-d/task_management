...
| `ARG VITE_*` | ビルド引数 | ビルド時に静的ファイルへ埋め込む `VITE_*` 変数 | 秘匿情報は渡さない |
...
sequenceDiagram
    ...
    DEV->>DOCKER: docker build -f frontend/Dockerfile<br/>--build-arg VITE_*=... frontend
    DOCKER->>BLD: package.json / package-lock.json をコピー
    BLD->>BLD: npm ci
    DOCKER->>BLD: 残りのソース一式をコピー
    BLD->>BLD: npm run build（VITE_*を埋め込みdist/生成）
...
```mermaid
flowchart TB
    A["docker build --build-arg VITE_*=..."] --> B["builder: COPY package.json package-lock.json"]
    B --> C["builder: npm ci"]
...
| 引数/入力 | `ARG VITE_*`（各既定値は§3の設定項目を参照）。`COPY package.json package-lock.json` を先に行いレイヤキャッシュを利用 |
| 出力 | `/app/dist` 配下のビルド成果物 |
| 失敗条件 | `npm ci` の依存解決失敗、TypeScript型エラー等によるビルド失敗 |
| 処理内容 | 1. `ARG VITE_*`を宣言 2. `ENV VITE_*=$VITE_*`でビルド環境へ反映 3. `COPY package.json package-lock.json ./` 4. `RUN npm ci` 5. `COPY . .` 6. `RUN npm run build` |
