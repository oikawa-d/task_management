...
| タイミング攻撃対策 | 該当なし |
| レート制限 | なし |
| fail-close方針 | PostgreSQL/Redis接続不能時は `503 SERVICE_UNAVAILABLE`。DB更新を先にコミットするため、Redis失効が失敗しても無効化済みのDB状態を維持する。部分失効はERROR監査後に再実行する |
...
| 区分 | 内容 | 影響 |
|------|------|------|
| 採用 | DB更新を先にコミットし、その後にRedis失効を実行する。Redis失敗時もDBは無効化済みのままとし、部分失効は同じuser_idで再実行できる | staleな認証状態が残っても、無効化済みのDB判定でフェイルセーフに倒せる |
