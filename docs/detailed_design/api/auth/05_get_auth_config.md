...
| google_login_enabled | boolean | 不可 | `GOOGLE_LOGIN_ENABLED=true`かつ`GOOGLE_CLIENT_ID`/`GOOGLE_CLIENT_SECRET`が設定されている場合のみ`true` |
...
sequenceDiagram
    ...
    SET-->>R: auth_mode, GOOGLE_LOGIN_ENABLED, GOOGLE_CLIENT_ID/SECRET有無, COOKIE_NAME_CSRF
...
flowchart TB
    ...
    D -->|"Yes"| F["google_login_enabled=true"]
    D -->|"No"| G["google_login_enabled=false"]
    F --> H["AuthConfigResponse組み立て"]
    E --> H
    G --> H
    H --> I["200 + Cache-Control: no-store"]
```

## 6. 関数詳細

### 6.1 `api/app/api/routers/auth_router.py :: get_auth_config`

| 項目 | 内容 |
|------|------|
| シグネチャ | `async def get_auth_config(response: Response, settings: BackendSettings = Depends(get_backend_settings)) -> AuthConfigResponse` |
| 引数 | `response`（ヘッダ設定用）、`settings`（DI） |
| 戻り値 | `AuthConfigResponse`（200） |
| 送出例外 | なし（通常運用時） |
| 処理内容 | 1. `response.headers["Cache-Control"] = "no-store"`を設定 2. `auth_service.get_auth_config(settings)`を呼び出して`AuthConfigResponse`を返す |
| 副作用 | なし（レスポンスヘッダ設定のみ） |

### 6.2 `api/app/service/auth_service.py :: get_auth_config`

| 項目 | 内容 |
|------|------|
| シグネチャ | `def get_auth_config(settings: BackendSettings | None = None) -> AuthConfigResponse` |
| 引数 | `settings`：設定値。省略時は`get_backend_settings()`から取得 |
| 戻り値 | `AuthConfigResponse` |
| 処理内容 | `auth_mode`、`GOOGLE_LOGIN_ENABLED`とGoogle client id/secretの設定状況から`google_login_enabled`、CSRF Cookie名を組み立てる |
| 副作用 | なし |

### 6.3 `api/app/core/config.py :: get_backend_settings`

| 項目 | 内容 |
|------|------|
| シグネチャ | `@lru_cache def get_backend_settings() -> BackendSettings` |
| 引数 | なし |
| 戻り値 | `BackendSettings`（pydantic-settingsによる環境変数バインディング済みインスタンス） |
...
DB repositoryは経由しない。serviceの設定Facadeを経由し、設定値の参照のみで完結する。
...
| fail-close方針 | 外部ストアに依存しないため503は発生しない。設定読み込み自体の失敗は起動時に検出されるため、稼働中の異常系は基本的に発生しない設計。Googleログインを無効化した場合はボタンだけでなくOAuth APIも無効化される |
...
| 3 | 単体 | 明示的に無効化した場合 | `GOOGLE_LOGIN_ENABLED=false`かつGoogle設定あり | `google_login_enabled=false` | `test_config_reports_google_login_disabled_when_flag_is_false` |
| 4 | 結合 | `AUTH_MODE=session`起動時 | 環境変数設定 | `200 {"auth_mode": "session", ...}` | `test_auth_config_endpoint_session_mode` |
| 5 | 結合 | `AUTH_MODE=jwt`起動時 | 環境変数設定 | `200 {"auth_mode": "jwt", ...}` | `test_auth_config_endpoint_jwt_mode` |
| 6 | 結合 | レスポンスヘッダ確認 | 任意モード | `Cache-Control: no-store`が付与される | `test_auth_config_endpoint_no_store_header` |
| 7 | 結合 | 秘密情報の非包含確認 | 任意モード | レスポンスボディに`GOOGLE_CLIENT_SECRET`等の文字列を含まない | `test_auth_config_endpoint_no_secret_leak` |
