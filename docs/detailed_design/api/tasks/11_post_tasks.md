...
| 区分 | 内容 | 影響 |
|------|------|------|
| 確定 | 未所属タスク（`project_id=NULL`）に`assignee_id`を設定できない制約は、issue #40で現行方針のまま確定した（将来的な拡張の余地は残す） | - |
| 確定 | `sp_create_task`のposition採番は`fn_next_task_position`をそのまま流用する。issue #40で当該DB関数を`project_id IS NOT DISTINCT FROM`によりNULL許容へ拡張することに確定したため、リポジトリ層でのSQL直接組み立ては不要（[../../database/08_db_functions.md §3.3](../../database/08_db_functions.md)参照） | - |
| 確定 | 固定プレースホルダキーによる未所属タスク全体の直列化は、issue #40で学習用途としては現行方針のまま確定した。本番運用規模でのロック競合増加は将来のスケーラビリティ課題として別途検討する | - |
