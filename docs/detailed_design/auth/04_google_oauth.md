...
| 要検討 | `GOOGLE_LOGIN_ENABLED=false`時にOAuth開始・callback・exchangeの直接呼び出しをどう扱うか（404/405等）が基本設計に未記載。本実装では認証設定APIで無効状態を通知するが、エンドポイントの追加拒否は行わない | UI非表示とサーバー側の機能停止を一致させるにはHTTP仕様の確定が必要 |
| 要検討 | Rate Limitキーの`PTTL`が負値（キー消失またはTTL未設定）を返した場合は`Retry-After: 1`として扱う。Redis異常として503にするか、別の再試行方針にするかは未確定 | レート制限状態の異常時に案内する待機時間と障害検知の扱いに影響 |
