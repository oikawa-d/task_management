...
| 区分 | 内容 | 影響 |
|------|------|------|
| 確定 | アクセストークンのdenylistは採用せず、強制ログアウト後も最大`ACCESS_TOKEN_TTL_SECONDS`（既定900秒）有効とする。即時遮断が必要な場合はDBの`is_active`を再確認するstatus APIを用いる | 遅延を利用者へ表示し、Redisのsession/refresh失効は直ちに実施する |
