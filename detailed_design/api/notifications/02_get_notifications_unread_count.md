...
| AUTH_MODE差異 | 差異なし |
| 冪等性 | あり（GET） |
| レート制限 | 対象外（ポーリング間隔は `VITE_NOTIFICATION_POLL_INTERVAL_MS` によりフロント側で制御。バックエンド側のレート制限は設けない。§11参照） |
...
| タイミング攻撃対策 | 該当なし |
| レート制限 | なし。ポーリング間隔の制御は `VITE_NOTIFICATION_POLL_INTERVAL_MS` によるフロント側の責務であり、バックエンドはこれを信頼する。悪意あるクライアントが間隔を無視して高頻度リクエストする場合の防御は本設計の対象外（要検討） |
