...
少なくとも1フィールドの指定を必須とする（空ボディは422）。ヘッダ：`X-CSRF-Token`（sessionモードの更新系で必須）。
...
| 422 | `VALIDATION_ERROR` | ボディが空、`name` が制約外等 | 入力内容に誤りがあります | |
...
sequenceDiagram
    ...
    FE->>R: PATCH /api/projects/{project_id} {name?, description?}
    ...
    S->>RP: update(project.id, name?, description?)
    RP->>PG: "UPDATE projects SET name=COALESCE(:name,name), description=... WHERE id=:pid RETURNING *"
...
flowchart TB
    ...
    D -->|"OK"| E["pydanticでname/descriptionを検証（最低1項目必須）"]
...
| 引数 | `project`: 更新対象（`require_project_owner` 済み） / `payload`: `name`, `description` の部分更新値 |
| 戻り値 | `ProjectSummary` |
| 送出例外 | `ServiceUnavailableError`（DB接続不能）→503 |
| 処理内容 | 1. `payload.model_dump(exclude_unset=True)` で指定されたフィールドのみ抽出 2. `project_repository.update(db, project.id, **fields)` を呼び出す 3. 更新後の `Project` を取得し、`member_count` / `task_counts` / `is_owner` を集計・付与して `ProjectSummary` を返す |
...
| 引数 | `project_id`: 対象 / `**fields`: `name` および/または `description` |
| 戻り値 | 更新後の `Project` |
...
| projects | UPDATE | `id=:project_id` | 指定フィールドのみ更新、`updated_at` はトリガ更新 |
...
| `ProjectUpdateRequest` | （モデルバリデータ） | `name`・`description`のいずれも未指定なら422 | `zod.object({...}).refine(has at least one field)` |
...
`AUTH_MODE=session` / `jwt` の両方で No.4・No.5・No.6を実施する。
