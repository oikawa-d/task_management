...
| 目的 | 管理者が任意のプロジェクト（自身がオーナー・所属メンバーであるかを問わない）を削除する。所属メンバー・タスク・タスクコメントもあわせて削除する |
| 認証 | 必要 |
| 認可 | admin固定 |
...
| レート制限 | 対象外 |
| トランザクション境界 | `DELETE FROM projects WHERE id=:project_id` 一文。`project_members` / `tasks` / `task_comments` はPostgreSQLの外部キー `ON DELETE CASCADE` により同一トランザクション内でDBエンジンが自動削除する（[05_delete_project.md](../projects/05_delete_project.md) と同一のCASCADE範囲） |
...
| 非所属・不存在時の応答 | 非所属memberは存在有無を隠して404、対象自体が無ければ404 | admin視点では「所属」概念がないため、`project_id` が存在しなければ404、存在すれば常に削除可 |
...
| CASCADE削除範囲 | 同一（`project_members` / `tasks` / `tasks`配下の `task_comments`） | 同一（本APIも [05_delete_project.md §9](../projects/05_delete_project.md) と全く同じ範囲） |
| 呼び出し元画面 | プロジェクト詳細画面の削除ボタン（オーナー/admin向け） | 管理者ユーザー管理画面のプロジェクト一覧タブ（他人が所有するプロジェクトも一覧から直接削除する運用を想定） |
| 監査ログの重み付け | 通常操作の一部として記録 | 「他者が所有するプロジェクトを第三者（管理者）が強制的に削除する」操作であるため、`owner_id`（対象プロジェクトの元オーナー）を必ずログへ含め、通常削除より重点的に扱う（11章参照） |
...
sequenceDiagram
    ...
    R->>S: delete_project(actor, project_id)
    ...
    alt 対象が存在しない
        ...
        PG-->>RP: project行
        RP-->>S: Project（削除前のowner_id/member_count等をログ用に保持）
...
flowchart TB
    ...
    F -->|"存在する"| G["削除前情報を保持<br/>（owner_id/member_count/task_counts）"]
    G --> H["project_repository.delete(project_id)"]
    H --> I["監査ログ出力"]
    I --> J["204 No Content"]
...
| 処理内容 | 1. `require_admin`・`verify_csrf` を通過 2. `admin_project_service.delete_project(db, actor, project_id)` を呼び出す 3. `204 No Content` を返す |
| 副作用 | なし（副作用はservice層に委譲） |

### 6.2 `service/admin_project_service.py :: delete_project`

| 項目 | 内容 |
|------|------|
| シグネチャ | `async def delete_project(db: AsyncSession, actor: CurrentUser, project_id: UUID) -> None` |
| 引数 | `actor`: 実行者（admin） / `project_id`: 削除対象 / `db`: DBセッション |
| 戻り値 | なし |
| 送出例外 | `NotFoundError`（404）、`ServiceUnavailableError`（DB接続不能）→503 |
| 処理内容 | 1. `project_repository.get_by_id(project_id)` で存在確認と、監査ログ用の `owner_id` を取得する（存在しなければ `NotFoundError`） 2. `project_repository.delete(project_id)` を呼び出す 3. `commit` する 4. `owner_id` を含めて監査ログを出力する（11章） |
| 副作用 | `projects` のDELETE。DBの外部キー `ON DELETE CASCADE` により `project_members` / `tasks` / `task_comments` が連鎖削除される（アプリ層で個別にDELETE文を発行しない）。所属メンバー・元オーナーへの通知は行わない（基本設計に規定なし。13章参照） |

### 6.3 `repository/project_repository.py :: get_by_id` / `delete`

[../projects/05_delete_project.md §6.3](../projects/05_delete_project.md) で定義済みの `delete(db, project_id)` をそのまま再利用する。`get_by_id` も `04_patch_project.md` / `05_delete_project.md` で定義済みの既存関数を再利用し、本API専用の新規リポジトリ関数は追加しない（所属確認 `exists` は本APIでは呼び出さない点のみが差異）。
...
flowchart LR
    ...
    R --> S["admin_project_service.delete_project"]
    S --> RP1["project_repository.get_by_id"]
    S --> RP2["project_repository.delete"]
...
flowchart TB
    ...
    B -->|"1件"| C["DELETE FROM projects WHERE id=:pid"]
    C --> D["FK CASCADE: project_membersのproject_id一致行を削除"]
    C --> E["FK CASCADE: tasksのproject_id一致行を削除"]
    E --> F["FK CASCADE: 削除されたtasksのtask_commentsを削除"]
    D --> G["COMMIT"]
    F --> G
...
削除範囲外（CASCADEされない）：`users`（`projects.owner_id` は `ON DELETE RESTRICT` だがユーザー削除時の制約でありプロジェクト削除には無関係）、`login_history`（プロジェクトと無関係のテーブルのため対象外）。CASCADE範囲そのものは [../projects/05_delete_project.md §9](../projects/05_delete_project.md) と完全に同一である。
...
| 観点 | 内容 |
|------|------|
| ログ出力 | 監査ログ対象（破壊的操作。かつ管理者による他者所有リソースへの強制操作のため通常の削除より重点的に扱う）。`actor.id`（実行した管理者）, `project_id`, 削除対象プロジェクトの `owner_id`（元オーナー）, 削除時点の `member_count`/`task_counts`（削除前に取得）, `X-Request-ID` を **WARN** で出力する（[../projects/05_delete_project.md](../projects/05_delete_project.md) はINFO/WARNいずれでも可としているのに対し、本APIは常にWARN以上とする） |
| ユーザー列挙対策 | admin専用APIのため対象外 |
| タイミング攻撃対策 | 該当なし |
| レート制限 | なし |
| 破壊的操作の確認 | フロント側で削除確認ダイアログを表示し、対象プロジェクト名・元オーナー名を明示する（本APIはバックエンド側の取り消し不可を前提とし、Undo機能は提供しない） |
| 元オーナー・メンバーへの通知 | 本APIは削除の事後通知（メール等）を行わない。管理者による強制削除であることをメンバーへ周知する手段が必要かは基本設計に規定がなく、要検討（13章） |
...
|----|------|--------|------|----------|-----------------|
| 1 | 単体 | serviceがrepository.deleteを1回呼び出す | repositoryをモック | `delete(project.id)` 呼び出しを検証、`project_member_repository.exists` は未呼び出し | `test_admin_delete_project_calls_repository_without_membership_check` |
| 2 | 結合 | adminは他人が所有するプロジェクトを204で削除できる | 一般ユーザーが所有し、adminは非所属のプロジェクトを用意 | `204`、`projects`/`project_members`/`tasks`/`task_comments`が全て削除される | `test_admin_delete_project_cascades_all_related_rows_for_non_member_project` |
...
| 5 | 結合 | 削除後の再削除は404（冪等性） | 同一project_idへ2回目のDELETE | `404 NOT_FOUND` | `test_admin_delete_project_idempotent_second_call_returns_404` |
| 6 | 結合 | sessionモードでCSRFヘッダ欠落は403 | `X-CSRF-Token`なし | `403 CSRF_INVALID` | `test_admin_delete_project_missing_csrf_session_mode` |
| 7 | 結合 | 監査ログにowner_id（元オーナー）が出力される | ログ出力をキャプチャして検証 | ログレコードに `actor_id`/`project_id`/`owner_id` が含まれる | `test_admin_delete_project_audit_log_contains_original_owner` |
...
| 区分 | 内容 | 影響 |
|------|------|------|
| 要検討 | 管理者による強制削除時、元オーナー・所属メンバーへの通知（メール等）を行うかは基本設計に規定がない。本書では「行わない」を前提としたが、運用上の要望次第では `auth_service` のメール送信基盤（[../../auth/06_token_mail.md](../../auth/06_token_mail.md)）を流用した通知機能の追加が要検討 |
| 要検討 | 削除確認（誤操作防止）のUI仕様は `basic_design/05_frontend.md` および `screen/10_admin_users.md` 側の管轄であり、本APIの入出力には影響しないため詳細は割愛した |
