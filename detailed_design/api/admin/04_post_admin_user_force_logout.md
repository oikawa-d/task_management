...
| 要検討 | jwtモードでaccess tokenを即時失効させる手段（denylist方式の`jti`をRedisに保持する等）を将来導入するかは基本設計`03_auth.md` §4.5で「本設計では採用しない」と明記されているため、本APIの15分残存制約は仕様として確定している。管理者向け画面でこの制約をどう文言表示するかは画面設計側の要検討事項 |
