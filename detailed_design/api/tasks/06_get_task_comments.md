...
| 区分 | 内容 | 影響 |
|------|------|------|
| 要検討 | コメント件数の上限・ページングの要否が基本設計に明記されていない。本設計では全件返却（`?page`等は導入しない）としたが、1タスクあたりのコメント数が多くなる運用を想定する場合は `basic_design/04_api.md` §1 のページング規約（`PAGINATION_DEFAULT_PER_PAGE` 等の設定項目名を用いる想定）に合わせた追加が必要 | 大量コメント時のレスポンスサイズ・表示パフォーマンス |
