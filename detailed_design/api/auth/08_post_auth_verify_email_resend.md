...
| レート制限 | 対象：`emailverify_sent:{user_id}` による再送間隔制限（既定60秒、`EMAIL_VERIFY_RESEND_INTERVAL_SECONDS`）。IPベースの追加レート制限は基本設計に明記なし（要検討） |
...
| レート制限（メール爆撃対策） | `emailverify_sent:{user_id}` により同一ユーザーへの連続送信を60秒間隔に制限する。IPアドレス単位の追加レート制限は基本設計に記載がなく本設計でも新設しない（要検討） |
...
このAPIは `AUTH_MODE` に依存しないため両モードでの重複実施は不要。IPレート制限は未実装のため該当ケースは網羅しない（§13参照）。

## 13. 不明点・要検討事項

| 区分 | 内容 | 影響 |
|------|------|------|
| 要検討 | IPアドレス単位の追加レート制限（ユーザー単位のみでは、多数のメールアドレスへ一括送信されるメール爆撃を防げない）の要否 | 中（外部からのメール爆撃・SMTP送信コスト増大のリスク） |
