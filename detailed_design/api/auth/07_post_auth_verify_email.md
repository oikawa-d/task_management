...
| レート制限 | 対象外（トークン自体が高エントロピーなランダム値であり、総当たりが非現実的なため専用のRedisレート制限キーは設けない。汎用のIPベースレート制限があればそちらに従う＝要検討） |
...
| レート制限 | 専用のレート制限は設けない（トークンの高エントロピー性に依拠）。総当たり対策として汎用IPレート制限の適用を要検討 |
...
| 要検討 | トークン総当たり対策として汎用IPレート制限をこのエンドポイントに適用するかどうかは基本設計に明記がない | 低〜中（token_urlsafe(32)の高エントロピー性により実害は小さいと想定） |
