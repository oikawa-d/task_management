...
| レート制限 | 対象外（`code`は60秒TTLかつ32バイトのランダム値で推測不可能なため）。総当たり対策として要検討 |
...
| レート制限 | 対象外。`code`推測は事実上不可能（32バイトランダム値・60秒TTL） |
...
| 区分 | 内容 | 影響 |
|------|------|------|
| 要検討 | ハンドオフ取得後の`user_id`再検証と`login_history`記録を同一トランザクションにまとめるか、独立した操作のままとするかが基本設計に明記されていない | 記録漏れ（refresh発行は成功したがhistory記録が失敗するケース）が起きた場合の挙動に影響。本ファイルでは独立操作として設計した |
