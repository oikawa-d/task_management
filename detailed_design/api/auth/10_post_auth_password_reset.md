...
| レート制限 | 対象外（トークンの高エントロピー性に依拠。`09_post_auth_password_forgot.md` と同方針） |
| トランザクション境界 | `UPDATE users SET password_hash = ...` の1文。Redisのトークン消費・全セッション/リフレッシュ失効はアプリケーションレベルで直列実行し、PostgreSQL更新とは別トランザクション（Redis操作はPostgreSQLトランザクションに含まれない） |
...
| 要検討 | `09_post_auth_password_forgot.md` §13で指摘した「同時に複数の `pwreset:*` トークンが有効になり得る」点について、本APIでの消費時は指定されたトークンのみを `GETDEL` するため、他の未使用トークンは失効せずTTLまで残存する | 低〜中（他のトークンも本人しか知り得ない値のため実害は限定的だが、意図した挙動か要確認） |
