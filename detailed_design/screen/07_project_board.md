...
|------------------------|----------|------|------------|
| 404 `NOT_FOUND`（プロジェクト） | 「プロジェクトが見つかりません」全画面メッセージ | 遷移なし（`/`へのリンクを提示） | ダッシュボードへ戻るリンク |
...
| 要検討 | `409 TASK_CONFLICT` 時に「ロールバックせずボードを再取得する」方針は、[00_overview.md §5.2](../../basic_design/00_overview.md#52-タスクのドラッグ＆ドロップによるステータス変更)の「ロールバック（元の列に戻す）」という記述と厳密には異なる（本書は409を「最新状態の反映」、それ以外の4xx/5xxを「ロールバック」として区別した）。基本設計側の記述が409を含む全失敗を指しているか要確認 |
