...
PR作成後は作成した時点で作業完了とせず、CIの全チェックが成功しても、ユーザーの明示承認を得るまでmerge・Issue closeを実施しないでください。PRを作成したエージェントは、自身が作成したPRをmergeしてはいけません。merge・Issue closeはユーザー承認後に別の操作主体が実施します。CIが失敗した場合はmergeせず、原因を修正してから再度確認してください。
