...
| No | 要件 | 内容 |
|----|------|------|
| N-1 | 定期通知 | 毎朝10時（`APP_TIMEZONE` 基準）に、**翌日10時までが期限**の未完了タスクを抽出し、そのタスクの担当者へ通知を作成する |
...
| 項目 | 内容 |
|------|------|
| タスクの期限 | 「翌日10時まで」「終了時刻の変更」を表現するため、期限を日付のみ（`due_date`）ではなく**日時**（`due_at`）で保持する |
| 基準タイムゾーン | 「朝10時」「当日」の判定基準を `APP_TIMEZONE`（既定 `Asia/Tokyo`）としてサーバー側で統一する。DBはUTC（`TIMESTAMPTZ`）保存のままとする |
...
8. タスク期限のアプリ内通知機能（`due_at` 移行 → 通知テーブル・通知API → 通知UI → `batch` コンテナによる毎朝10時の定期通知）
