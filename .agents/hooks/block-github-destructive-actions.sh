...
#
# 【検出できないケース(既知の限界、#390で費用対効果を検討済み)】この判定は誤検知の抑制が
# 目的であり、意図的な回避を防ぐものではない。以下のような形は `gh` の直前に区切り文字が
# 来ないため検出されない。
#   - クォートで包まれたコマンド: bash -c "gh pr merge 123 --squash" / sh -c 'gh issue close 123'
#     → 未対応。引数文字列を再帰的に解析する必要があり、実行位置(このコマンド自身が本当に
#       `bash -c` として実行されるか)と単なる文字列引数(echoの引数等、#388の誤検知の原因)を
#       安全に区別する実装コストが高い。誤って後者まで拾うと#388の再発になる一方、
#       `bash -c` でgh操作を書く経路は通常の操作導線ではほぼ出現しないため、
#       費用対効果の観点で対応を見送った。
#   - コマンド文字列をファイルへ書き出してから実行する形: cat > run.sh ...; bash run.sh
#     → 未対応。hookはBashツールのcommand文字列のみを見ており、ファイルの中身は読まない。
#       ファイル内容を読む実装はhookの責務(コマンド文字列の検査)を超えるため対応しない。
# `sudo` / `env VAR=値` によるプレフィックス付き呼び出し、および `xargs` 経由の呼び出しは
# #390 で対応し、`gh_token_is_known_wrapper` (lib/gh-command-parse.sh) が検出する。
